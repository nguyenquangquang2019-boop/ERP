...
<<<<<<< HEAD
                ...
                if (!string.IsNullOrEmpty(bienSo))
                {
                    ...
                    {
                        // Thay 'LoadDataPhuongTien()' bằng tên hàm load dữ liệu bảng của bạn (ví dụ: LoadData(), HienThi(),...)
                        LoadDataXe();
                    }
                }
=======
...
<<<<<<< HEAD
            ...
            if (dgvData.CurrentRow != null)
            {
                // Lấy Biển số xe từ dòng đang chọn (khớp với tên cột colBienSoXe trong bảng của bạn)
                string bienSoXe = dgvData.CurrentRow.Cells["colBienSoXe"].Value?.ToString();
                string loaiXe = dgvData.CurrentRow.Cells["colLoaiXe"].Value?.ToString(); // Hoặc cột tên tài xế tùy ý hiển thị
...
<<<<<<< HEAD
                            ...
                            if (rowsAffected > 0)
                            {
                                ...
                                // Gọi lại hàm load dữ liệu bảng xe của bạn ở đây 
                                // (Thay 'LoadDataPhuongTien' bằng tên hàm thực tế bạn đang dùng để tải lại DataGridView)
                               LoadDataXe();
                            }
                            else
                            {
